...
def node(node_id: str, node_type: str = "COMPONENT", **attrs: object) -> GraphNode:
    return GraphNode(node_id, node_type, attrs, authority=AuthorityStatus.IMPLEMENTED)
...
class HyperGraphTests(unittest.TestCase):
    def test_valid_graph_has_stable_digest(self) -> None:
        ...
        self.assertEqual(graph.to_dict()["graph_digest"], graph.to_dict()["graph_digest"])
    ...
    def test_hyperedge_requires_distinct_nodes(self) -> None:
        with self.assertRaises(ContractError):
            HyperEdge(
                "HYPER:1",
                "DECISION_IMPACT",
                (HyperParticipant("NODE:A", "DECISION"), HyperParticipant("NODE:A", "RISK")),
            )
    ...
    def test_hyperedge_participant_integrity(self) -> None:
        ...
        graph.add_hyperedge(HyperEdge(
            "HYPER:1",
            "DECISION_IMPACT",
            (HyperParticipant("NODE:A", "DECISION"), HyperParticipant("NODE:C", "RISK")),
        ))
    ...
    def test_transitive_dependencies(self) -> None:
        ...
        self.assertEqual(graph.transitive_dependencies("NODE:A"), ("NODE:B", "NODE:C"))
...
class EventLedgerTests(unittest.TestCase):
    def base_event(self, ledger: EventLedger, event_id: str, event_type: str, causation_id: str | None = None) -> LedgerEvent:
    ...
    def test_hash_chain_and_replay(self) -> None:
        ...
        count = ledger.replay(lambda state, _: state + 1, 0)
    ...
    def test_out_of_order_sequence_rejected(self) -> None:
        event = LedgerEvent(
            event_id="EVENT:1", sequence=2, project_id="PROJECT:TEST", agent_id="AGENT:A",
            session_id="SESSION:A", workstream_id="WORKSTREAM:A", objective_id="OBJECTIVE:A",
            correlation_id="CORRELATION:A", event_type="HELLO", occurred_at=NOW,
            main_sha="a" * 40, base_sha="a" * 40, branch="test", authority_ceiling="ARCHITECTURE_ONLY",
            resource_scopes=("repo:test",), semantic_scopes=("architecture:test",), payload={},
        ).signed()
        with self.assertRaises(ContractError):
    ...
    def test_unknown_causation_rejected(self) -> None:
        ledger = EventLedger()
        event = self.base_event(ledger, "EVENT:1", "WORK_STARTED", "EVENT:MISSING")
        with self.assertRaises(ContractError):
            ledger.append(event)
...
class CoordinationTests(unittest.TestCase):
    ...
    def test_overlapping_write_claim_rejected(self) -> None:
        ...
        registry.acquire_claim(ScopeClaim(
            "CLAIM:A", "SESSION:A", AccessMode.WRITE, ("repo:swiss:docs",), ("contract:v2",), ClaimState.ACTIVE, NOW
        ))
        with self.assertRaises(CoordinationError):
            registry.acquire_claim(ScopeClaim(
                "CLAIM:B", "SESSION:B", AccessMode.WRITE, ("repo:swiss:docs:file",), ("contract:v2",), ClaimState.ACTIVE, NOW
            ))
    ...
    def test_parallel_non_overlapping_claims_allowed(self) -> None:
        ...
        registry.acquire_claim(ScopeClaim(
            "CLAIM:B", "SESSION:B", AccessMode.WRITE, ("repo:swiss:src",), ("runtime:v2",), ClaimState.ACTIVE, NOW
        ))
    ...
    def test_read_read_overlap_allowed(self) -> None:
        ...
        registry.open_session(session("SESSION:A", "AGENT:A"))
        registry.open_session(session("SESSION:B", "AGENT:B"))
        for claim_id, session_id in (("CLAIM:A", "SESSION:A"), ("CLAIM:B", "SESSION:B")):
            registry.acquire_claim(ScopeClaim(
                claim_id, session_id, AccessMode.READ, ("store:authority",), (), ClaimState.ACTIVE, NOW
            ))
        self.assertEqual(len(registry.claims), 2)
    ...
    def test_fencing_takeover_rejects_stale_writer(self) -> None:
        ...
        first = registry.acquire_lease(
            lease_id="LEASE:A", session_id="SESSION:A", scope="store:authority", acquired_at=NOW, expires_at=LATER
        )
        second = registry.acquire_lease(
            lease_id="LEASE:B", session_id="SESSION:B", scope="store:authority", acquired_at=MUCH_LATER, expires_at="2026-08-30T12:00:00Z"
        )
        self.assertGreater(second.fencing_token, first.fencing_token)
        with self.assertRaises(CoordinationError):
            registry.assert_fence("store:authority", first.fencing_token, MUCH_LATER)
        registry.assert_fence("store:authority", second.fencing_token, MUCH_LATER)
    ...
    def test_lease_cannot_be_stolen_before_expiry(self) -> None:
        ...
        registry.acquire_lease(
            lease_id="LEASE:A", session_id="SESSION:A", scope="store:authority", acquired_at=NOW, expires_at=MUCH_LATER
        )
        with self.assertRaises(CoordinationError):
            registry.acquire_lease(
                lease_id="LEASE:B", session_id="SESSION:B", scope="store:authority", acquired_at=LATER, expires_at="2026-08-30T12:00:00Z"
            )
    ...
    def test_close_session_releases_claims_and_leases(self) -> None:
        ...
        registry.open_session(session("SESSION:A"))
        registry.acquire_claim(ScopeClaim(
            "CLAIM:A", "SESSION:A", AccessMode.WRITE, ("repo:test",), (), ClaimState.ACTIVE, NOW
        ))
        registry.acquire_lease(
            lease_id="LEASE:A", session_id="SESSION:A", scope="repo:test", acquired_at=NOW, expires_at=MUCH_LATER
        )
        registry.close_session("SESSION:A", LATER)
        self.assertEqual(registry.claims["CLAIM:A"].state.value, "RELEASED")
...
class ContextPackTests(unittest.TestCase):
    def build_pack(self) -> ContextPack:
        return ContextPack(
            ...
            payload={"api_key": "super-secret", "safe": "value", "nested": {"token": "abc"}},
        ).signed()

    def test_contextpack_redacts_secrets(self) -> None:
        pack = self.build_pack()
        payload = pack.to_dict()["payload"]
    ...
    def test_contextpack_freshness(self) -> None:
        pack = self.build_pack()
        pack.assert_fresh(main_sha="a" * 40, event_watermark=pack.event_watermark, projection_revision="c" * 64)
        with self.assertRaises(StaleContextError):
            pack.assert_fresh(main_sha="f" * 40, event_watermark=pack.event_watermark, projection_revision="c" * 64)

    def test_secret_value_pattern_redaction(self) -> None:
        value = redact_secrets({"message": "credential ghp_abcdefghijklmnopqrstuvwxyz1234"})
        self.assertNotIn("ghp_", value["message"])


class AssuranceAndCompilerTests(unittest.TestCase):
    def graph_with_critical_node(self, tested: bool = True, owned: bool = True) -> TemporalHyperGraph:
        graph = TemporalHyperGraph()
        attrs = {"critical": True}
        if owned:
            attrs["owner"] = "TEAM"
        ...
        if tested:
            graph.add_edge(GraphEdge("EDGE:TEST", "TESTED_BY", "NODE:CRITICAL", "TEST:CRITICAL"))
        return graph

    def test_release_candidate_when_p0_p1_closed(self) -> None:
        graph = self.graph_with_critical_node()
        invariant = InvariantSpec("INV:1", "critical property", Severity.P0, "TEAM", ("TEST:CRITICAL",))
        evidence = TestEvidence("TEST:CRITICAL", ResultState.PASS, "CI:1", NOW)
        gap = Gap("GAP:1", "residual", Severity.P2, 2, 2, 2, 2, 2, "TEAM", "metric", "guard", "later", (), "TEST:CRITICAL", "run", "MIGRATION")
    ...
    def make_task(self, task_id: str, deps: tuple[str, ...] = ()) -> TaskSpec:
        return TaskSpec(
            task_id, "objective", "why", ("input",), ("output",), deps, ("NODE:A",),
            ("EDGE:A",), ("file",), "TEAM", "risk", ("step",), ("test",),
            ("attack",), ("security",), ("evidence",), "rollback", ("done",),
        )
    ...
    def test_program_frontier_and_digest(self) -> None:
        ...
        checkpoint = CheckpointSpec(
            "CP:1", "checkpoint", ("entry",), ("TASK:1",), ("test",), ("evidence",),
            ("exit",), "MISSION", "rollback",
        )
        program = ImplementationProgram("PROGRAM:1", "G-0001", (first, second), (checkpoint,))
        self.assertEqual(program.validate(), ())
        self.assertEqual(program.executable_frontier(()), ("TASK:1",))
        self.assertEqual(program.executable_frontier(("TASK:1",)), ("TASK:2",))
    ...
    def test_program_dependency_cycle_detected(self) -> None:
        ...
        checkpoint = CheckpointSpec("CP:1", "checkpoint", ("entry",), ("TASK:1",), ("test",), ("evidence",), ("exit",), "MISSION", "rollback")
        program = ImplementationProgram("PROGRAM:1", "G-0001", (one, two), (checkpoint,))
        self.assertTrue(any("cycle" in error for error in program.validate()))

    def test_cos_registry_requires_all_dimensions(self) -> None:
        with self.assertRaises(ContractError):
            ProjectionRegistry((COSDimension("L0", "Visual", ProjectionStatus.ACTIVE, "purpose"),))

    def test_deferred_dimension_requires_trigger(self) -> None:
        with self.assertRaises(ContractError):
            COSDimension("L10", "Similarity", ProjectionStatus.DEFERRED_TRIGGER, "purpose")
...
class MutationLoopGuardTests(unittest.TestCase):
    def test_duplicate_successful_create_is_suppressed(self) -> None:
        ...
        first = MutationAttempt(
            "create_issue", "repo:swiss", MutationClass.IRREVERSIBLE_CREATE,
            "graph-refactor-v2-root", "strategy:create", "SUCCESS", "ISSUE:293",
        )
        guard.record(first)
        decision = guard.assess(
            action="create_issue", target_scope="repo:swiss",
            mutation_class=MutationClass.IRREVERSIBLE_CREATE,
            idempotency_key="graph-refactor-v2-root", strategy_id="strategy:create",
        )
    ...
    def test_placeholder_idempotency_key_rejected(self) -> None:
        ...
        with self.assertRaises(ContractError):
            guard.assess(
                action="create_issue", target_scope="repo:swiss",
                mutation_class=MutationClass.IRREVERSIBLE_CREATE,
                idempotency_key="none", strategy_id="strategy:create",
            )
    ...
    def test_identical_failure_budget_forces_strategy_change_then_stuck(self) -> None:
        ...
        for index in range(2):
            guard.record(MutationAttempt(
                "create_issue", "repo:swiss", MutationClass.IRREVERSIBLE_CREATE,
                f"attempt-{index}", "strategy:broken", "NO_PROGRESS",
            ))
        self.assertEqual(
            guard.assess(
                action="create_issue", target_scope="repo:swiss",
                mutation_class=MutationClass.IRREVERSIBLE_CREATE,
                idempotency_key="attempt-2", strategy_id="strategy:broken",
            ),
            LoopDecision.CHANGE_STRATEGY,
        )
        guard.record(MutationAttempt(
            "create_issue", "repo:swiss", MutationClass.IRREVERSIBLE_CREATE,
            "attempt-2", "strategy:broken", "NO_PROGRESS",
        ))
        self.assertEqual(
            guard.assess(
                action="create_issue", target_scope="repo:swiss",
                mutation_class=MutationClass.IRREVERSIBLE_CREATE,
                idempotency_key="attempt-3", strategy_id="strategy:broken",
            ),
...
class CompilerIntegrationTests(unittest.TestCase):
    def test_compiler_emits_complete_public_safe_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            graph = json.loads((out / "system_graph.json").read_text(encoding="utf-8"))
            self.assertGreaterEqual(len(graph["nodes"]), 80)
            self.assertGreaterEqual(len(graph["edges"]), 100)
            self.assertEqual(len(graph["hyperedges"]), 2)
            self.assertEqual(len(list((out / "projections").glob("L*.json"))), 20)
            self.assertEqual((out / "event_ledger.jsonl").read_text(encoding="utf-8").count("\n"), 5)
            context = json.loads((out / "contextpack.json").read_text(encoding="utf-8"))
