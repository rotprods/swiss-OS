...
Origin: selectively salvaged from closed PR #414 during `CONVERGENCE_ONLY`; rebased onto current canonical main rather than merging stale lineage.
...
13. `CONVERGENCE_ONLY` is active and the proposed mutation opens a new material feature/domain wave instead of reducing live drift, salvage debt, ownership ambiguity, or North-Star execution debt.
...
If three consecutive engineering changes do not create a measurable North-Star-relevant capability, remove a blocker, or improve a defined SLO/invariant, stop architecture expansion and re-evaluate priorities against real acquisition signal.

During `CONVERGENCE_ONLY`, the default answer to a new architecture feature is STOP unless it is required to reconcile existing accepted value or unblock the employment North Star.
...
MEP-2.0 chooses and chains safe productive routes. This policy decides when a route must stop, repair, downgrade, or request human input instead of fabricating progress.

Neither contract lowers authority, evidence, privacy, outbound, provider, or user-authorization gates.
