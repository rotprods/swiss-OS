...
SOCIAL_OR_INFRA_HOSTS = frozenset({
    ...
    "google.com", "www.google.com", "maps.google.com", "googleapis.com",
    "hotelleriesuisse.ch", "www.hotelleriesuisse.ch",
})
...
def extract_identity_candidates(html: bytes, page_url: str) -> dict[str, object]:
    ...
    for root in parser.jsonld:
        for node in _walk_jsonld(root):
            ...
            if kind.lower() not in {"hotel", "lodgingbusiness", "organization", "localbusiness"}:
                continue
...
def _fetch(url: str, *, timeout: float = 30.0) -> tuple[int, str, bytes]:
    request = Request(url, headers={"User-Agent": "SWITZERLAND_JOB_OS-ProviderIdentityEnrichment/1.0 (+review-only)"})
    try:
        with urlopen(request, timeout=timeout) as response:
            return int(getattr(response, "status", 200)), response.geturl(), response.read()
...
def enrich_batch(batch: Mapping[str, object], *, timeout: float = 30.0, delay: float = 0.25) -> dict[str, object]:
    ...
    for index, item in enumerate(items):
        ...
        seen.add(key)
        if _text(item.get("canonical_hotel_id")) or _text(item.get("matched_hotel_id")) or _text(item.get("allocated_hotel_id")):
        ...
        if _host(url) not in {"hotelleriesuisse.ch"}:
            raise ProviderIdentityEnrichmentError(f"unsupported provider detail host for {key}")
        if index and delay:
            time.sleep(delay)
    ...
    packet: dict[str, object] = {
        "schema_version": "PROVIDER-IDENTITY-ENRICHMENT-1.0",
...
def validate_packet(packet: Mapping[str, object]) -> tuple[str, ...]:
    violations: list[str] = []
    if packet.get("schema_version") != "PROVIDER-IDENTITY-ENRICHMENT-1.0":
    ...
    for result in results:
        if _text(result.get("canonical_hotel_id")) or _text(result.get("matched_hotel_id")) or _text(result.get("allocated_hotel_id")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    run = sub.add_parser("run"); run.add_argument("batch"); run.add_argument("--out", required=True); run.add_argument("--timeout", type=float, default=30.0); run.add_argument("--delay", type=float, default=0.25)
