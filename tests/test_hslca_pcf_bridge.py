...
def _page(position: int, record: dict[str, str]) -> dict[str, object]:
    ...
    return {
        ...
        "source_url": "https://www.hotelleriesuisse.ch/de/directory"
...
class HSLCAProofBridgeTests(unittest.TestCase):
    def test_enriches_exact_checkpoint_mtimes_inside_capture_window(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(
                out["pages"][0]["captured_at_basis"],
                "ATOMIC_CHECKPOINT_FILE_MTIME",
            )
            self.assertEqual(
                out["checkpoint_timestamp_provenance"]["pages_verified"], 2
            )
    ...
    def test_bridge_file_writes_enriched_capture(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            summary = bridge_file(
                capture_path, pages_dir=pages_dir, out_path=out_path
            )
