...
def _task_id(snapshot_id: str, snapshot_record_id: str, task_type: str) -> str:
    digest = hashlib.sha256(
        f"{snapshot_id}|{snapshot_record_id}|{task_type}".encode("utf-8")
    ).hexdigest()[:20]
    return f"CRM-{digest}"
...
def task_for_decision(decision: IngestDecision) -> TaskSpec | None:
    ...
    return TaskSpec(
        task_id=_task_id(decision.snapshot_id, decision.snapshot_record_id, task_type),
        scope_id=decision.snapshot_record_id,
        task_type=task_type,
        priority=priority,
        freshness_key=decision.snapshot_id,
        reason_code=reason_code,
    )


def enqueue_ingest_work(conn: sqlite3.Connection, decisions: list[IngestDecision]) -> dict[str, int]:
    """Create only missing scheduler work for actionable non-authoritative decisions."""
    created = 0
    skipped = 0
    no_task = 0
    ...
    return {
        "CREATED": created,
        "SKIPPED_EXISTING_OR_COMPLETE": skipped,
        "NO_TASK_REQUIRED": no_task,
        **{f"CREATED_{key}": value for key, value in sorted(by_type.items())},
    }
