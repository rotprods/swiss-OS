...
class MutationLoopGuard:
    """Suppress duplicate creates and terminate repeated no-progress strategies.

    The kernel keeps this representation in memory. Production callers persist
    attempts as ledger events and rebuild the guard by replay. This class never
    grants authority, H-ID allocation, or permission for an external action.
    """

    def __init__(self, *, max_identical_strategy_attempts: int = 3) -> None:
        if isinstance(max_identical_strategy_attempts, bool) or not isinstance(
            max_identical_strategy_attempts, int
        ):
    ...
    def assess(
        ...
        *,
        ...
        if mutation_class in {
            MutationClass.IRREVERSIBLE_CREATE,
            MutationClass.IRREVERSIBLE_EXTERNAL,
        } and idempotency_key.strip().lower() in {"none", "na", "n/a", "unknown"}:
            raise ContractError("irreversible mutation requires a real idempotency key")

        probe = MutationAttempt(
            action,
            target_scope,
            mutation_class,
            idempotency_key,
        ...
        if failures >= self.max_identical_strategy_attempts:
            return LoopDecision.STUCK_LOOP
        if failures == self.max_identical_strategy_attempts - 1:
