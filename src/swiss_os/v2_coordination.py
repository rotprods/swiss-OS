...
def _legacy_claim_candidates(event, claims):
    """Strict causal fallback for lifecycle events written before claim causation.

    Some historical release events were intentionally emitted by a successor session
    while closing the predecessor claim. Therefore session equality alone is not
    authoritative. A candidate must share project/workstream/objective, already exist
    at event time, and cover every semantic scope declared by the event. If the event
    has no semantic scopes, exact session identity remains required. The caller accepts
    the fallback only when exactly one candidate survives.
    """
    ...
    for claim in claims:
        ...
        if event_scopes:
            if not event_scopes.issubset(claim_scopes):
                continue
        elif _text(claim, "session_id") != _text(event, "session_id"):
            continue
...
def derive_claim_lifecycle(events, claims):
    ...
    for event in ordered_events:
        ...
        if explicit_ids:
            ...
            binding_mode = "LEGACY_CAUSAL_TEMPORAL_SEMANTIC"
