...
def test_p1_b03_adversarial_confusables_stay_distinct():
    art = json.loads(ART.read_text(encoding="utf-8"))
    by_key = {x["source_record_key"]: x for x in art["decisions"]}
    assert by_key["MD-8b6c2242e364c6468f0a"]["suggested_hotel_ids"] == ["H-0489"]
    assert by_key["MD-92655bbac05436c7e861"]["reason_code"] == "CURRENT_DISTINCT_PROPERTY_NAMES_DOMAINS_AND_PROVIDER_IDENTITIES"
    assert by_key["MD-cb01bf8880ce6c4e3d7c"]["reason_code"] == "OFFICIAL_PROVIDER_LISTS_PILATUS_KULM_AND_BELLEVUE_AS_SEPARATE_HOTELS"
