...
The controller is deliberately side-effect free. It records progress evidence,
blocks repeated no-progress routes, applies activation budgets and asks the MEP
planner for the next safe route. Actual work remains inside WOP waves.
"""
...
class ActivationJournal:
    ...
    def route_repetition_without_progress(
        ...
        *,
        ...
        relevant = [event for event in self.events if event.route == route]
        ...
        if len(tokens) == 1 and len(artifacts) <= 1:
            return LoopGuardResult(
                ...
                reason=(
                    f"{route} repeated {len(tail)} times without a new progress token "
                    "or durable artifact"
                ),
            )
        return LoopGuardResult(False)
    ...
    def action_repetition_without_progress(
        ...
        *,
        ...
        if len(tokens) == 1 and len(artifacts) <= 1:
            return LoopGuardResult(
                triggered=True,
                blocked_route=tail[-1].route or None,
                repeated_count=len(tail),
                reason=(
                    f"action class {action_class} repeated {len(tail)} times without progress"
                ),
            )
        return LoopGuardResult(False)
...
def _blocked_candidate(candidate: RouteCandidate, reason: str) -> RouteCandidate:
    blocked = tuple(sorted({*candidate.blocked_capabilities, f"LOOP_GUARD:{candidate.route.value}"}))
    return replace(
        candidate,
...
def plan_chained_next(
    ...
    *,
    ...
    """Plan NEXT after disabling routes/actions trapped in no-progress loops."""
    ...
    # Issue mutation cannot be selected indirectly through REPO_ENGINEERING when
    # the issue-create budget/loop guard is locked. Engineering remains possible;
    # only the issue-create action is prohibited and surfaced as capability state.
