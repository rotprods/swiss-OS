...
def choose_meta_route(c: MetaCapabilities) -> MetaDecision:
    ...
    if not c.crm_universe_complete:
        ...
        # Directory evidence can and should be built independently of an API
        # capture. This is the required fallback when the discover.swiss key is
        # unavailable and is also useful in parallel with structured capture.
        if not c.member_directory_manifest_complete and (c.web_research or c.member_directory_evidence):
            used = tuple(key for key, value in (
        ...
        if c.discover_capture_valid and c.member_directory_manifest_complete and not c.source_scope_reconciled:
            return MetaDecision(
                ExecutionMode.DEGRADED_CANARY,
                MetaRoute.SOURCE_SCOPE_RECONCILIATION,
                "Both source sets exist; SSR-1.0 is the next hard gate before freeze/export.",
                "META",
                capabilities_used=("discover_capture_valid", "member_directory_manifest_complete"),
                next_fallback_routes=_fallbacks(MetaRoute.MEMBER_DIRECTORY_MANIFEST, MetaRoute.EXACT_CURRENT_REFRESH),
            )

        if c.source_scope_reconciled and c.frozen_candidate and not c.ingest_records_ready:
            return MetaDecision(
                ExecutionMode.DEGRADED_CANARY,
                MetaRoute.FROZEN_CANDIDATE_EXPORT,
                "Source scope is reconciled; deterministic candidate→CMI export unlocks mass ingest without allocating IDs.",
                "META",
        ...
        if c.ingest_records_ready and c.constrained_db_write:
            return MetaDecision(
                ExecutionMode.DEGRADED_CANARY,
                MetaRoute.MASS_INGEST_STAGING,
                "CMI records and constrained staging write are available; classify/anti-join/schedule at scale.",
                "META",
                capabilities_used=("ingest_records_ready", "constrained_db_write"),
                next_fallback_routes=_fallbacks(MetaRoute.EXACT_CURRENT_REFRESH, MetaRoute.TERMINAL_MAPPING),
            )
        ...
        if (c.exact_current_refresh_backlog > 0 or c.unresolved_source_records > 0) and c.web_research:
            return MetaDecision(
                ...
                "Source records still require current evidence; exact-current refresh precedes terminal mapping.",
                "META",
                capabilities_used=("web_research",),
                next_fallback_routes=_fallbacks(MetaRoute.TERMINAL_MAPPING, MetaRoute.MEMBER_DIRECTORY_MANIFEST, MetaRoute.ENGINEERING_QA),
            )
        ...
        if c.source_scope_reconciled and c.reconcile_required > 0 and c.exact_current_refresh_backlog == 0:
            return MetaDecision(
                ...
                "Required evidence is present; remaining reconciliations should terminate into canonical/alias/exclusion mappings.",
                "OPERATIONAL",
    ...
    if c.crm_universe_complete and c.scheduler_task_available:
        return MetaDecision(
            ExecutionMode.DEGRADED_CANARY,
            MetaRoute.NEXT_GOAL_SCHEDULER,
            "The CRM-universe hard gate is complete; return control to the global North-Star scheduler for the next dependency.",
            "META",
    ...
    if c.drive_mount_read and not c.native_sheets_read:
        return MetaDecision(
            ...
            "Native Sheets access is unavailable but the authenticated Drive mount can still rehydrate persistent artifacts.",
            "META",
            capabilities_used=("drive_mount_read",),
            next_fallback_routes=_fallbacks(MetaRoute.RECOVERY_PERSISTENCE, MetaRoute.ENGINEERING_QA),
        )
    ...
    if c.github_write and c.github_ci:
        return MetaDecision(
            ...
            "Operational routes are blocked; Git/CI work is allowed only to close measured execution or integrity debt.",
            "META",
    ...
    if c.library_write or c.drive_create_only_write:
        return MetaDecision(
            ...
            "No higher-value operational route is available; persist recoverable state instead of losing work.",
            "META",
            capabilities_used=tuple(key for key, value in (("library_write", c.library_write), ("drive_create_only_write", c.drive_create_only_write)) if value),
        )

    return MetaDecision(
        ExecutionMode.BLOCKED_P0,
        MetaRoute.NO_SAFE_ROUTE,
        "No safe route can currently reduce the bottleneck without violating a hard gate.",
        "NONE",
