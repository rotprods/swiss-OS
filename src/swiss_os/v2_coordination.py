...
def _legacy_claim_candidates(event, claims):
    """Compatibility for historical lifecycle events written before causation binding.

    The fallback is deliberately strict: session + workstream + objective must identify
    exactly one durable claim. It never guesses by token, branch, chronology or scope.
    """
    keys = ("session_id", "workstream_id", "objective_id")
    return [
        claim
        for claim in claims
        if all(_text(claim, key) == _text(event, key) for key in keys)
    ]
...
def derive_claim_lifecycle(events, claims):
    ...
    for event in ordered_events:
        ...
        if explicit_ids:
            ...
            candidates = _legacy_claim_candidates(event, claims)
            if len(candidates) != 1:
                errors.append(
...
def reduce_coordination(events, claims):
    ...
    if events:
        ordered = sorted(
            (_text(event, "occurred_at"), _text(event, "event_id")) for event in events
        )
...
def build_context_pack(
    *,
    ...
    if not isinstance(relevant_paths, list) or not relevant_paths or not all(
        isinstance(item, str) and item.strip() for item in relevant_paths
    ):
