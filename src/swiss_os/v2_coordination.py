...
def validate_event(event):
    ...
    for key in (
        "canonical_hotel_mutation_allowed",
        "h_id_allocation_allowed",
        "outbound_allowed",
    ):
...
def scopes_overlap(left: Sequence[str], right: Sequence[str]) -> bool:
    return bool(
        {x.strip() for x in left if isinstance(x, str) and x.strip()}
        & {x.strip() for x in right if isinstance(x, str) and x.strip()}
    )


def detect_claim_collisions(claims):
    active = [claim for claim in claims if claim.get("state") in ACTIVE_CLAIM_STATES]
    output = []
    for index, left in enumerate(active):
        for right in active[index + 1 :]:
            ...
            resource_overlap = scopes_overlap(
                left.get("resource_scopes", [])
                if isinstance(left.get("resource_scopes"), list)
                else [],
                right.get("resource_scopes", [])
                if isinstance(right.get("resource_scopes"), list)
                else [],
            )
            semantic_overlap = scopes_overlap(
                left.get("semantic_scopes", [])
                if isinstance(left.get("semantic_scopes"), list)
                else [],
                right.get("semantic_scopes", [])
                if isinstance(right.get("semantic_scopes"), list)
                else [],
            )
...
def _causation_claim_ids(event: Mapping[str, object]) -> list[str]:
    ...
    return [
        item.split(":", 1)[1]
        for item in value
        if isinstance(item, str) and item.startswith("claim:") and len(item) > 6
    ]
...
def _legacy_claim_binding(event, claims):
    """Resolve pre-causation lifecycle records using strongest durable evidence first.

    Historical lifecycle events did not always carry ``claim:<id>`` causation. Some
    release events were emitted by a successor session while closing the predecessor,
    so session identity alone cannot be authoritative. The fallback therefore ranks
    only deterministic evidence already present in the durable corpus:

    1. exact lifecycle timestamp (claimed/released/superseded_at);
    2. exact session identity;
    3. a unique semantic candidate.

    Project/workstream/objective and semantic scope remain hard filters. Any tie at
    the strongest available tier is returned intact so the caller fails closed.
    """
    ...
    for claim in claims:
        ...
        broad_candidates.append(claim)

    timestamp_field = CLAIM_EVENT_TIMESTAMP_FIELD.get(event_type)
    if timestamp_field and occurred_at:
        timestamp_matches = [
            claim
            for claim in broad_candidates
            if _text(claim, timestamp_field) == occurred_at
        ]
        if timestamp_matches:
            return timestamp_matches, "LEGACY_LIFECYCLE_TIMESTAMP"
    ...
    if session_id:
        session_matches = [
            claim
            for claim in broad_candidates
            if _text(claim, "session_id") == session_id
        ]
        if session_matches:
            return session_matches, "LEGACY_SESSION_IDENTITY"

    return broad_candidates, "LEGACY_UNIQUE_SEMANTIC"


def _legacy_claim_candidates(event, claims):
    """Compatibility wrapper returning only the candidates."""
    candidates, _ = _legacy_claim_binding(event, claims)
    return candidates
    ...
    for event in ordered_events:
        ...
        if explicit_ids:
            ...
            if claim_id not in by_id:
                errors.append(
                    f"UNKNOWN_CLAIM_CAUSATION:{_text(event, 'event_id')}:{claim_id}"
                )
                continue
            binding_mode = "EXPLICIT_CAUSATION"
        else:
            candidates, binding_mode = _legacy_claim_binding(event, claims)
            if len(candidates) != 1:
                errors.append(
                    f"UNBOUND_CLAIM_LIFECYCLE_EVENT:{_text(event, 'event_id')}:{len(candidates)}"
                )
                continue
...
def reduce_coordination(events, claims):
    ...
    for event in events:
        errors.extend(
            f"{_text(event, 'event_id') or '<unknown>'}:{error}"
    ...
    for claim in claims:
        errors.extend(
            f"{_text(claim, 'claim_id') or '<unknown>'}:{error}"
            for error in validate_claim(claim)
        )

    claim_states, claim_lifecycle, lifecycle_errors = derive_claim_lifecycle(
        events, claims
    )
    ...
    if events:
        ordered = sorted(
            (_text(event, "occurred_at"), _text(event, "event_id"))
            for event in events
        )
...
def build_context_pack(
    *,
    ...
    state_refs,
    relevant_paths,
    relevant_scope_revision,
    blockers,
    next_safe_actions,
):
    if not _is_git_sha(base_main_sha):
        raise CoordinationError(
            "base_main_sha must be lowercase 40-hex Git object ID"
        )
    if not _is_sha256(relevant_scope_revision):
        raise CoordinationError(
            "relevant_scope_revision must be lowercase sha256"
        )
    ...
    if (
        ...
        or not all(isinstance(item, str) and item.strip() for item in relevant_paths)
    ):
...
def validate_context_pack(
    ...
    *,
    ...
    current_relevant_scope_revision,
    ...
    if (
        current_authority_revision is not None
        and pack.get("authority_revision") != current_authority_revision
    ):
        errors.append("STALE_AUTHORITY_REVISION")
    if pack.get("context_pack_revision") != sha256_json(
        {key: value for key, value in pack.items() if key != "context_pack_revision"}
    ):
