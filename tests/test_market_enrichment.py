...
class MarketEnrichmentTests(unittest.TestCase):
    ...
    def test_official_site_requires_explicit_website_signal(self):
        ...
        <a href="https://vendor.example/">Booking technology</a>
        <a href="https://hotel.example/">Website</a>
        '''
        self.assertEqual(
            me.external_official_candidates("https://www.hotelleriesuisse.ch/hotel-x", hs),
            ["https://hotel.example/"],
        )
    ...
    def test_route_discovery_is_same_site_and_typed(self):
        ...
        <a href="/staff-accommodation">Staff accommodation</a>
        <a href="https://jobs.vendor.example/posting">External jobs</a>
        '''
    ...
    def test_structured_jobposting_is_strong_t1_signal(self):
        ...
        self.assertEqual(jobs[0]["source_url"], "https://hotel.example/jobs/chef")
    ...
    def test_public_url_guard(self, getaddrinfo):
        ...
        with self.assertRaises(ValueError):
            me.validate_public_https_url("http://example.com")
        with self.assertRaises(ValueError):
            me.validate_public_https_url("https://127.0.0.1")
        with self.assertRaises(ValueError):
            me.validate_public_https_url("https://user:pass@example.com")

    def test_aggregate_requires_exact_coverage_and_remains_fail_closed(self):
        manifest = {
        ...
        def rec(rid):
            return {
                "record_id": rid,
                "e07_vacancy": {"state": "CAREERS_ROUTE_NOT_DISCOVERED", "structured_openings_count": 0, "opening_routes": [], "careers_routes": []},
                "official_site": {"url": None},
                "e08_housing": {"state": "STAFF_HOUSING_RESEARCH_PENDING"},
                "e09_people": {"state": "PEOPLE_RESEARCH_PENDING"},
                "e10_channel": {"state": "CHANNEL_RESEARCH_PENDING", "spontaneous_application_policy": "NOT_OBSERVED"},
                "e17_application": {"candidate_truth_block_required": True},
            }

        packets = [
            {"source_records_sha256": "abc", "records": [rec("a")]},
            {"source_records_sha256": "abc", "records": [rec("b")]},
        ]
    ...
    def test_source_contains_engine_and_security_locks(self):
        text = open("src/swiss_os/market_enrichment.py", encoding="utf-8").read()
        for marker in (
            '"e07_vacancy"', '"e08_housing"', '"e09_people"', '"e10_channel"',
            '"e11_intelligence"', '"e12_graph"', '"e14_scheduler"', '"e15_score"',
            '"e16_candidate_truth"', '"e17_application"', '"e18_qa"', '"e19_observability"',
            '"e20_recovery"', '"e21_delivery"', '"e22_security"',
            '"candidate_truth_block_required": True', '"final_send_ready": False',
            '"outbound": "CLOSED"', '"send_allowed": 0', '"canonical_id_reservations": 0',
        ):
