...
Converts one explicitly scoped directory capture into the evidence manifest required
by SSR-1.0. Partial/historical caches remain discovery evidence and can never
assert ``coverage_complete=true``.
"""
...
def _normalized_url_parts(value: object) -> tuple[str, str, str]:
    raw = _text(value)
    if not raw:
        return "", "", ""
    parsed = urlsplit(raw)
...
def normalize_detail_url(value: object) -> str:
    """Normalize entity-detail identity while ignoring non-identity query/fragment."""

    raw = _text(value)
    if not raw:
        return ""
    scheme, netloc, path = _normalized_url_parts(raw)
    return urlunsplit((scheme, netloc, path, "", ""))


def normalize_source_url(value: object) -> str:
    """Normalize an evidence/surface URL while preserving scope-defining query.

    Query parameters are sorted for determinism but not discarded: directory
    filters can define a materially different source surface.
    """
...
def _record_from_capture(
    ...
    *,
    ...
    return {
        ...
        "source_url": normalize_source_url(record.get("source_url") or page_source_url),
        "page_id": page_id,
...
def build_member_directory_manifest(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Output is always non-authoritative evidence. ``coverage_complete`` is true
    only when the complete-capture contract passes with zero violations.
    """
    ...
    for page_index, page in enumerate(pages_payload, start=1):
        ...
        try:
            page_source_url = normalize_source_url(page.get("source_url"))
        except ValueError as exc:
            page_source_url = ""
        ...
        if not page_source_url:
            violations.append(f"PAGE_SOURCE_URL_MISSING:{page_id or page_index}")
        elif page_source_url in seen_page_urls:
            violations.append(f"DUPLICATE_PAGE_SOURCE_URL:{page_source_url}")
        seen_page_urls.add(page_source_url)
    ...
    if claim == CoverageClaim.COMPLETE:
        ...
        if seen_page_positions != set(range(1, expected_pages + 1)):
    ...
    manifest["manifest_sha256"] = _sha256(dict(manifest))
    return manifest
...
def validate_member_directory_manifest(payload: Mapping[str, Any]) -> ManifestValidation:
    ...
    if _text(payload.get("records_sha256")) != _sha256(normalized_records):
