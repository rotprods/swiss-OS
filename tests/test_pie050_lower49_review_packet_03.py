...
PACKET3 = Path("docs/state/PIE050_LOWER49_REVIEW_PACKET_03_2026-08-30.json")
NEU = "MD-33d867e983644585e4b2"


def test_packet03_lineage_scope_cumulative_nonoverlap_and_fail_closed_effects():
    queue = json.loads(QUEUE.read_text(encoding="utf-8"))
    p1 = json.loads(PACKET1.read_text(encoding="utf-8"))
    p2 = json.loads(PACKET2.read_text(encoding="utf-8"))
    data = json.loads(PACKET3.read_text(encoding="utf-8"))
    ...
    assert data["parent_git_sha"] == "cffdb3259acd703e4c6250f42e49597a37d9a761"
    ...
    assert data["reviewed_cumulative"] == 30
    assert data["pending_lower49_after"] == 19
    assert set(keys) <= set(queue["source_record_keys"])
    assert not (set(keys) & prior_keys)
    assert NEU not in keys
    assert len(prior_keys | set(keys)) == 30
    assert hashlib.sha256("\n".join(sorted(keys)).encode()).hexdigest() == data["reviewed_source_record_keys_sha256"]

    allowed_tiers = {
        "CURRENT_PUBLIC_ACCOMMODATION_LISTING",
    ...
    assert effect["similarity_collision_reviews_cleared_cumulative"] == 30
...
def test_packet03_content_hash_detects_silent_rewrite():
    data = json.loads(PACKET3.read_text(encoding="utf-8"))
