...
class T(unittest.TestCase):
    ...
    def test_explicit_release_is_event_derived(self):
        ...
        self.assertEqual(
            projection["claim_lifecycle"][0]["binding_mode"], "EXPLICIT_CAUSATION"
        )
    ...
    def test_terminal_event_exposes_stale_claim_file(self):
        ...
        self.assertEqual(projection["active_claim_ids"], [])
        self.assertIn(
            "CLAIM_STATE_DRIFT:CL-1:ACTIVE!=RELEASED", projection["violations"]
        )
    ...
    def test_legacy_ambiguous_lifecycle_fails_closed(self):
        one = claim("ONE", token=1, state="RELEASED")
        ...
        self.assertIn(
            "UNBOUND_CLAIM_LIFECYCLE_EVENT:E-AMB:2", projection["violations"]
        )
    ...
    def test_unknown_explicit_claim_causation_fails_closed(self):
        lifecycle_event = event(
            ...
            "unknown",
            "CLAIM_RELEASED",
            causation=["claim:MISSING"],
        )
        projection = reduce_coordination([lifecycle_event], [claim("KNOWN")])
        self.assertIn(
            "UNKNOWN_CLAIM_CAUSATION:E-UNKNOWN:MISSING", projection["violations"]
        )
    ...
    def test_context_descendant_head_without_scope_drift_is_valid(self):
        ...
        pack = build_context_pack(
            project_id="P",
            base_main_sha=GIT_SHA,
            authority_revision="A",
            projection=projection,
            state_refs=[],
            relevant_paths=["ARCHITECTURE.md"],
            relevant_scope_revision=SCOPE_REV,
            blockers=[],
            next_safe_actions=[],
        )
        self.assertEqual(
    ...
    def test_state(self):
        ...
        self.assertIn(
            "V2_ARCHITECTURE_STATE_MUST_NOT_OPEN_OUTBOUND",
